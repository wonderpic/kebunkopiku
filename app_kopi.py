...
st.markdown("""
    <div class="kotak-warning-petani">
        ⚠️ **PENTING UNTUK PETANI:** Aplikasi ini berjalan mandiri di HP Anda. Agar catatan tidak hilang saat di-refresh, pastikan Anda menggunakan tombol <b>📥 Unduh Cadangan</b> di menu Data Kebun secara berkala.
    </div>
""", unsafe_allow_html=True)

# --- 🌟 SISTEM DAFTAR MEMORI TEKS STATIS MANDIRI (ANTI-TERCAMPUR) 🌟 ---
if 'kebun_list_murni' not in st.session_state:
    st.session_state.kebun_list_murni = []

# --- NAVIGASI MENU UTAMA (3 MENU PALING STABIL) ---
menu = st.radio("Pilih Menu:", ["📊 Data Kebun", "📅 Jadwal Kerja", "🌱 Tambah Blok"], horizontal=True)
st.write("---")

# 1. MENU: TAMBAH BLOK (LENGKAP DENGAN INPUT LOKASI & KETINGGIAN MDPL)
if menu == "🌱 Tambah Blok":
    ...
    if submit_button and nama_blok:
        nama_kembar = any(k.get('Blok') == nama_blok for k in st.session_state.kebun_list_murni)
        if nama_kembar:
            ...
            st.session_state.kebun_list_murni.append(new_block)
...
# 2. MENU: DATA KEBUN
    ...
    if not st.session_state.kebun_list_murni:
        st.info("📲 Belum ada data kebun Anda. Silakan isi data baru di menu '🌱 Tambah Blok' untuk mulai mencatatkan kebun Anda.")
    else:
        total_pohon = sum(int(k.get('Jumlah_Pohon', 0)) for k in st.session_state.kebun_list_murni)
        st.metric(label="Total Semua Pohon Kopi Dikelola", value=f"{total_pohon} Batang")
        
        # Fitur download cadangan data ke HP petani agar data abadi selamanya
        df_download = pd.DataFrame(st.session_state.kebun_list_murni)
        ...
        for idx, row in enumerate(st.session_state.kebun_list_murni):
            st.markdown(f"""
            ...
            if st.button(f"🗑️ Hapus {row.get('Blok')}", key=f"hapus_{row.get('Blok')}_{idx}"):
                st.session_state.kebun_list_murni.pop(idx)
                st.rerun()

# 3. MENU: JADWAL KERJA (🌟 URUTAN GLOBAL TANGGAL TERDEKAT LINTAS BLOK 100% SUKSES ACCURATE)
    ...
    if not st.session_state.kebun_list_murni:
        ...
        for row in st.session_state.kebun_list_murni:
